...
import { buildDemoFrames } from '@/features/tutorial/demoScript';
import { DEMO_DEFAULT_SPEED, DEMO_SPEEDS, DemoTransport } from '@/features/tutorial/DemoControls';
import { useAppTranslation } from '@/hooks/useAppTranslation';
import { playSfx, tapFeedback } from '@/services/feedback';
import { theme } from '@/theme';

/**
 * The lesson frames are GENERATED by the real rules engine (traceMove) from a
 * fixed teaching position — see demoScript.ts — so the demo always agrees,
 * shell for shell, with the rules described in How to Play. This one turn
 * shows the Pasu (four) capture, a relay lap, and the empty-pit capture.
 */
      ...
      <View ref={boardRef} onLayout={onBoardLayout} style={styles.board}>
        ...
        <View style={styles.grid}>
          <DemoRow
            ...
            captureIndex={frame.captureRow === 'top' ? frame.captureIndex : undefined}
            boardRef={boardRef}
            registry={pitCenters}
          ...
          <DemoRow
            ...
            captureIndex={frame.captureRow === 'bottom' ? frame.captureIndex : undefined}
            boardRef={boardRef}
            registry={pitCenters}
            layoutVersion={layoutVersion}
          />
...
function DemoRow({
  ...
  values: number[];
  /** Engine pit index of each display position (real-board orientation). */
  ...
  return (
    <View style={styles.row}>
      {pits.map((count, i) => (
        <DemoPit
          key={i}
          boardIndex={indices[i]}
          count={count}
          active={isActiveRow && activeIndex === i}
          capture={captureIndex === i}
          dim={!isActiveRow && captureIndex === undefined}
          boardRef={boardRef}
          registry={registry}
          layoutVersion={layoutVersion}
        />
